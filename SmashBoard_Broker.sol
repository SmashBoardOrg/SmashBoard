...
contract SmashBoard_Broker is Ownable {
    ...
    string public name = "SmashBoard Broker";      // Extended name of this contract
    uint256 public tokenPrice = 0;        // Set the fixed SmashBoard token price
    address public smashBoardAddr;            // Set the SmashBoard contract address
    uint256 public decimals = 18;         // The decimals to consider

    mapping (address => uint256) public sold;       // Map the SmashBoard token allcations
    ...
    constructor(
            address addrSmashBoard,
            address contractOwner

        ) public {
          smashBoardAddr = addrSmashBoard;
    ...
    function setSmashBoard(address _value) public onlyOwner {
      smashBoardAddr = _value;     // Set the ERC20 SmashBoard contract address default smashBoardAddr
    ...
    function withdrawPUB() public returns(bool) {
        ...
        require(block.timestamp > pubEnd);          // Require the SmashBoard_Broker to be over - actual time higher than end unixtime
        require(sold[msg.sender] > 0);              // Require the SmashBoard token balance to be sent to be higher than 0

        // Send ESS tokens to the contributors proportionally to their contribution/s
        if(!smashBoardAddr.call(bytes4(keccak256("transferTokens(address,uint256)")), msg.sender, sold[msg.sender])){revert();}
    ...
    function isVerified (address _addr) public view returns (bool) {
      return Token(smashBoardAddr).verified(_addr);
    }

    function isReserved (address _addr) public view returns (uint256) {
      return Token(smashBoardAddr).reserve(_addr);
    }
    ...
    function reserveTokens (address _addr, uint256 _amount) public onlyOwner {
        require(block.timestamp < pubEnd);          // Require the current unixtime to be lower than the END unixtime
        require(Token(smashBoardAddr).verified(msg.sender) == true);
        require(Token(smashBoardAddr).verified(_addr) == true);

        if(!smashBoardAddr.call(bytes4(keccak256("reserveTokens(address,uint256)")), _addr, _amount)){revert();}
    }

    function tokensDelivery(uint256 _amount, address _user) public onlyOwner {
        require(Token(smashBoardAddr).verified(msg.sender) == true);
        require(Token(smashBoardAddr).verified(_user) == true);
        require(Token(smashBoardAddr).reserve(_user) <= _amount);

        if(!smashBoardAddr.call(bytes4(keccak256("transferTokens(address,uint256)")), _user, _amount)){revert();}
        if(!smashBoardAddr.call(bytes4(keccak256("withdrawTokens(address,uint256)")), _user, _amount)){revert();}
